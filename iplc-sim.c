...
void iplc_sim_LRU_update_on_hit(int index, int assoc, int tag);
...
void iplc_sim_LRU_replace_on_miss(int index, int tag)
{
    //move every value down in the tag array
    //The MRU will be in front and the LRU element will be in the back
    for(int i=cache_assoc-2; i>=0; i--)
    {
 ...
 */
void iplc_sim_LRU_update_on_hit(int index, int assoc_entry, int tag)
{
    //move the most recently used element to the front after it has been used
    for(int i=assoc_entry - 1; i>=0; i--)
    {
        cache[index].tag[i+1] = cache[index].tag[i];
    }
    cache[index].tag[0] = tag;
...
int iplc_sim_trap_address(unsigned int address)
{
    ...
    cache_access++;
    int assoc_entry=0;
    //create a mask in order to find the index value
    int mask = 0;
    for(int i=0;i<cache_index; i++)
    {
        mask |=  (1 << i);
    }
    //calculate index and tag values
    index = (address >> cache_blockoffsetbits) & mask;
    tag = address >> (cache_index + cache_blockoffsetbits);
    printf("Address %x: Tag= %x, Index= %x \n", address, tag, index);
    //Check to see if the given tag is in the cache
    for(int i=0; i<cache_assoc; i++)
    {
        //if it is in the cache we hit
    ...
    {
        cache_hit++;
        iplc_sim_LRU_update_on_hit(index, assoc_entry, tag);
    }
...
void insert_nops(unsigned int stage, int count) {
    pipeline_t tmp = pipeline[stage];
    bzero(&(pipeline[stage]), sizeof(pipeline_t));
    if (stage == FETCH)
        for (int i = 0; i < count; ++i)
            iplc_sim_push_pipeline_stage();
    else insert_nops(stage - 1, count);
    pipeline[stage] = tmp;
}
...
void iplc_sim_push_pipeline_stage()
{
    ...
    if (pipeline[WRITEBACK].instruction_address) {
        ...
        bzero(&(pipeline[WRITEBACK]), sizeof(pipeline_t));
    }
    ...
    if (pipeline[DECODE].itype == BRANCH) {
        ...
        if (pipeline[FETCH].instruction_address) {
            unsigned int delta = pipeline[FETCH].instruction_address - pipeline[DECODE].instruction_address;
            if ((delta == 4) == branch_predict_taken) {
                insert_nops(FETCH, 1);
                --branch_count;
                iplc_sim_push_pipeline_stage();
                return;
    ...
    */
    if (pipeline[MEM].itype == LW) {
        if (!iplc_sim_trap_address(pipeline[MEM].stage.lw.data_address)) {
            ...
            insert_nops(MEM, CACHE_MISS_DELAY - 1);
        } else {
            printf("DATA HIT:\t Address 0x%x \n", pipeline[MEM].stage.lw.data_address);
            unsigned int dst = pipeline[MEM].stage.lw.dest_reg;
            if (pipeline[ALU].itype == RTYPE)
                if (dst == pipeline[ALU].stage.rtype.reg1 ||
                    dst == pipeline[ALU].stage.rtype.reg2_or_constant)
                        insert_nops(ALU, 1);
        }
    }
